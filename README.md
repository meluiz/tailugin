...
Tailugin is a collection of Tailwind CSS v4-ready utilities for high-level motion design and state-driven styling. It packages expressive `@utility` rules for enter/leave animations, easing presets, and semantic UI state variants so you can compose polished interactions with idiomatic Tailwind class names.

> **Compatibility**: Tailugin targets the Tailwind CSS v4 plugin architecture (`@utility`, `@custom-variant`, `@theme`). Earlier versions of Tailwind do not understand these directives.
...
# or
pnpm add tailugin tailwindcss@next
```
...
Tailugin also ships granular entry points if you only need a subset of utilities:
...
Each entry point resolves to the bundled CSS under `plugins/`.
...
```html
<button class="animate-in zoom-in-95 fade-in-0 duration-200 ease-out-quart disabled:opacity-50">
  Continue
</button>
```
